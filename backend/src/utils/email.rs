...
impl EmailService {
    pub fn from_config(config: &Config) -> Self {
        ...
        if svc.smtp_host.is_none() {
            tracing::warn!("SMTP not configured — emails will be printed to the log only");
        } else {
            tracing::info!(
                "SMTP configured: host={} port={} user={:?} from={}",
    ...
    pub async fn send_otp(&self, to: &str, username: &str, otp: &str) {
        ...
        if let Err(e) = self.send(to, subject, &body).await {
            ...
            // Fallback so the code is always recoverable from Railway logs
            tracing::warn!("=== OTP FALLBACK (SMTP failed) === to={to} otp={otp} ===");
        }
    }
    ...
    pub async fn send_password_reset(&self, to: &str, token: &str) {
        ...
        if let Err(e) = self.send(to, subject, &body).await {
            ...
            tracing::warn!("=== RESET FALLBACK (SMTP failed) === to={to} url={url} ===");
        }
    }
    ...
    async fn send(&self, to: &str, subject: &str, body: &str) -> anyhow::Result<()> {
        match (&self.smtp_host, &self.smtp_user, &self.smtp_password) {
            (Some(host), Some(user), Some(pass)) => {
                ...
                // Port 465 = implicit SSL (Gmail "SSL" option)
                // Port 587 = STARTTLS (Gmail "TLS" option) — default
                ...
                mailer.send(email).await
                    .map_err(|e| anyhow::anyhow!("SMTP send error (host={host} port={} user={user}): {e}", self.smtp_port))?;

                tracing::info!("Email sent to {to}: {subject}");
                Ok(())
            }
            _ => {
                // SMTP not fully configured — log so it's still usable in development
                tracing::info!("==== EMAIL (no SMTP configured) ====");
                tracing::info!("To: {to}");
